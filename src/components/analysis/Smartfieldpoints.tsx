...
export function SmartFieldPoints({
    ...
    return (
      <div key={point.id} className="flex gap-1 items-center">
        <Button
          ...
          className={`flex-1 h-7 text-[8px] justify-between px-2 ${isSet ? "border-primary/50 bg-primary/5" : ""}`}
        >
  ...
  return (
    <div className="space-y-3 p-3 rounded-md border-2 border-primary/20 bg-primary/5">
      ...
      {/* Main toggle button */}
      <Button onClick={onToggle} variant={isActive ? "default" : "outline"} className="w-full h-8 text-[10px]">
        ...
        <div className="space-y-3">
            ...
            {canDraw ? (
              ...
              <div>
                <p className="font-semibold mb-1">
                  Mark {Math.min(3, requiredVisible.length) - requiredSet} more visible point
                  {Math.min(3, requiredVisible.length) - requiredSet !== 1 ? "s" : ""}
                </p>
          ...
          {corners.length > 0 && (
            <div className="space-y-1.5">
              <Label className="text-[9px] font-semibold flex items-center justify-between">
                ...
                <span className="text-[8px] font-normal text-muted-foreground">Mark any visible corners</span>
              </Label>
              <div className="grid grid-cols-2 gap-1">{corners.map(renderPointButton)}</div>
            </div>
          )}
          ...
          {edges.length > 0 && (
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-1">{edges.map(renderPointButton)}</div>
            </div>
          )}
          ...
          {(penalties.length > 0 || center.length > 0) && (
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-1">{[...penalties, ...center].map(renderPointButton)}</div>
            </div>
          )}
          ...
          {boxes.length > 0 && setCount >= 4 && (
            <div className="space-y-1.5">
              <Label className="text-[9px] font-semibold flex items-center justify-between">
                ...
                <button onClick={() => setShowAll(!showAll)} className="text-[8px] text-primary hover:underline">
                  {showAll ? "Hide" : "Show"}
                </button>
              </Label>
              {showAll && <div className="grid grid-cols-2 gap-1">{boxes.map(renderPointButton)}</div>}
            </div>
          )}
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`font-semibold ${
                    setCount >= 8 ? "text-green-500" : setCount >= 5 ? "text-yellow-500" : "text-orange-500"
                  }`}
                >
                  {setCount >= 8 ? "Excellent" : setCount >= 5 ? "Good" : "Basic"}
                </span>
              </div>
