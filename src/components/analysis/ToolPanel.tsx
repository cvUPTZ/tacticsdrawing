...
interface ToolPanelProps {
  ...
  showPitch: boolean;
  onTogglePitch: (show: boolean) => void;
}
...
const ToolGroup = forwardRef<HTMLDivElement, ToolGroupProps>(
  ({ title, tools, currentTool, onToolChange, hasVideo, defaultOpen = false }, ref) => {
          ...
          <div className="grid grid-cols-2 gap-0.5 pt-1">
            {tools.map((tool) => (
              <Button
                ...
                className={cn("h-7 justify-start gap-1.5 text-[10px] px-2", currentTool === tool.id && "tool-active")}
...
export const ToolPanel = forwardRef<HTMLDivElement, ToolPanelProps>(
  (
    {
      currentTool,
      currentColor,
      ...
      <div className="glass-panel rounded-lg p-3 flex flex-col gap-2">
              ...
              <div className="flex gap-1">
                {ZONE_SHAPES.map((shape) => (
                  <Button
                    ...
                    className={cn("flex-1 h-7 px-2 gap-1", zoneShape === shape.id && "border-primary bg-primary/10")}
                  >
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center justify-between py-0.5">
            ...
            <Switch id="dashed-mode" checked={isDashed} onCheckedChange={onDashedChange} className="scale-75" />
          </div>
        </div>
        ...
        <div className="space-y-1">
          ...
          <Button variant="default" size="sm" onClick={onSave} className="w-full justify-start gap-2 text-[10px] h-7">
