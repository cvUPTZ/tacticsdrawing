...
export function solveCameraPose(
  ...
  const multiplier = extensiveSearch ? 5 : 1;
  const steps = [
    { iter: 1000 * multiplier, learnRate: 2.0, rotLearn: 0.1, fovLearn: 2.0 }, // Coarse
    { iter: 2000 * multiplier, learnRate: 0.5, rotLearn: 0.02, fovLearn: 0.5 }, // Refine
    { iter: 2000 * multiplier, learnRate: 0.05, rotLearn: 0.005, fovLearn: 0.1 }, // Fine
  ];
  ...
  for (const step of steps) {
    for (let i = 0; i < step.iter; i++) {
      ...
      let delta = 0;
      if (idx < 3)
        delta = (Math.random() - 0.5) * step.learnRate; // Position
      else if (idx < 5)
        delta = (Math.random() - 0.5) * step.rotLearn; // Rotation
